...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent3, NestedScrollingChild3, ScrollingView
{
        ...
        if (actionMasked != 0) {
            if (actionMasked != 1) {
                if (actionMasked != 2) {
                    ...
                    if (pointerIndex == -1) {
                        ...
                        if (this.mIsBeingDragged) {
                            ...
                            final int n4 = this.getScrollY() - scrollY;
                            final int[] mScrollConsumed = this.mScrollConsumed;
                            this.dispatchNestedScroll(mScrollConsumed[1] = 0, n4, 0, n3 - n4, this.mScrollOffset, 0, mScrollConsumed);
                            ...
                            this.mNestedYOffset += this.mScrollOffset[1];
                            if (b) {
                                final int n5 = n3 - this.mScrollConsumed[1];
                                this.ensureGlows();
                                final int n6 = scrollY + n5;
                                if (n6 < 0) {
                                    EdgeEffectCompat.onPull(this.mEdgeGlowTop, n5 / (float)this.getHeight(), motionEvent.getX(pointerIndex) / this.getWidth());
                                    if (!this.mEdgeGlowBottom.isFinished()) {
                                        this.mEdgeGlowBottom.onRelease();
                                    }
                                }
                                else if (n6 > scrollRange) {
                                    EdgeEffectCompat.onPull(this.mEdgeGlowBottom, n5 / (float)this.getHeight(), 1.0f - motionEvent.getX(pointerIndex) / this.getWidth());
                                    if (!this.mEdgeGlowTop.isFinished()) {
                ...
                if (Math.abs(a2) > this.mMinimumVelocity) {
                    ...
                    if (!this.dispatchNestedPreFling(0.0f, n8)) {
                        this.dispatchNestedFling(0.0f, n8, true);
                        this.fling(n7);
                    }
                }
    ...
    public boolean pageScroll(final int n) {
        ...
        mTempRect3.bottom = height + top;
        return this.scrollAndFocus(n, top, mTempRect3.bottom);
    }
