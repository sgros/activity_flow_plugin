...
public class MiscUtils
{
    ...
    public static void getPathFromData(final ShapeData shapeData, final Path path) {
        ...
        MiscUtils.pathFromDataCurrentPoint.set(initialPoint.x, initialPoint.y);
        for (int i = 0; i < shapeData.getCurves().size(); ++i) {
            ...
            if (controlPoint1.equals((Object)MiscUtils.pathFromDataCurrentPoint) && controlPoint2.equals((Object)vertex)) {
                path.lineTo(vertex.x, vertex.y);
            }
            else {
                path.cubicTo(controlPoint1.x, controlPoint1.y, controlPoint2.x, controlPoint2.y, vertex.x, vertex.y);
            }
            MiscUtils.pathFromDataCurrentPoint.set(vertex.x, vertex.y);
        }
