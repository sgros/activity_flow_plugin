...
abstract class Atom
{
    ...
    static final class ContainerAtom extends Atom
    {
        ...
        public ContainerAtom(final int n, final long endPosition) {
            super(n);
        ...
        public void add(final ContainerAtom containerAtom) {
            this.containerChildren.add(containerAtom);
        }
        
        public void add(final LeafAtom leafAtom) {
            this.leafChildren.add(leafAtom);
        }
        
        public ContainerAtom getContainerAtomOfType(final int n) {
            for (int size = this.containerChildren.size(), i = 0; i < size; ++i) {
                final ContainerAtom containerAtom = this.containerChildren.get(i);
                if (containerAtom.type == n) {
        ...
        public LeafAtom getLeafAtomOfType(final int n) {
            for (int size = this.leafChildren.size(), i = 0; i < size; ++i) {
                final LeafAtom leafAtom = this.leafChildren.get(i);
                if (leafAtom.type == n) {
                    return leafAtom;
                }
            }
        ...
        public String toString() {
            final StringBuilder sb = new StringBuilder();
            sb.append(Atom.getAtomTypeString(super.type));
            sb.append(" leaves: ");
    ...
    static final class LeafAtom extends Atom
    {
        ...
        public LeafAtom(final int n, final ParsableByteArray data) {
            super(n);
            this.data = data;
        }
    }
