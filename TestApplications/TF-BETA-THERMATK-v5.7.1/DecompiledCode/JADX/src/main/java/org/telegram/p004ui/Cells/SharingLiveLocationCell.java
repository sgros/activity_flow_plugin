...
import org.telegram.p004ui.LocationActivity.LiveLocation;
...
public class SharingLiveLocationCell extends FrameLayout {
    ...
    private Runnable invalidateRunnable = new C23711();
    ...
    public void onMeasure(int i, int i2) {
        super.onMeasure(MeasureSpec.makeMeasureSpec(MeasureSpec.getSize(i), 1073741824), MeasureSpec.makeMeasureSpec(AndroidUtilities.m26dp(this.distanceTextView != null ? 66.0f : 54.0f), 1073741824));
    }
    ...
    public void setDialog(MessageObject messageObject, Location location) {
        ...
        if (TextUtils.isEmpty(messageObject.messageOwner.media.title)) {
            ...
            Drawable drawable = getResources().getDrawable(C1067R.C1065drawable.pin);
            ...
            CombinedDrawable combinedDrawable = new CombinedDrawable(Theme.createSimpleSelectorCircleDrawable(AndroidUtilities.m26dp(40.0f), color, color), drawable);
        ...
        if (location != null) {
            ...
            if (charSequence2 != null) {
                if (distanceTo < 1000.0f) {
                    this.distanceTextView.setText(String.format("%s - %d %s", new Object[]{charSequence2, Integer.valueOf((int) distanceTo), LocaleController.getString(str3, C1067R.string.MetersAway)}));
                } else {
                    this.distanceTextView.setText(String.format("%s - %.2f %s", new Object[]{charSequence2, Float.valueOf(distanceTo / 1000.0f), LocaleController.getString(str, C1067R.string.KMetersAway)}));
                }
            } else if (distanceTo < 1000.0f) {
                this.distanceTextView.setText(String.format("%d %s", new Object[]{Integer.valueOf((int) distanceTo), LocaleController.getString(str3, C1067R.string.MetersAway)}));
            } else {
                this.distanceTextView.setText(String.format("%.2f %s", new Object[]{Float.valueOf(distanceTo / 1000.0f), LocaleController.getString(str, C1067R.string.KMetersAway)}));
            }
        } else if (charSequence2 != null) {
            this.distanceTextView.setText(charSequence2);
        } else {
            this.distanceTextView.setText(LocaleController.getString("Loading", C1067R.string.Loading));
        }
    }

    public void setDialog(LiveLocation liveLocation, Location location) {
        this.liveLocation = liveLocation;
        int i = liveLocation.f611id;
        String str = "50_50";
        Object user;
        if (i > 0) {
            user = MessagesController.getInstance(this.currentAccount).getUser(Integer.valueOf(i));
        ...
        GeoPoint position = liveLocation.marker.getPosition();
        ...
        if (location != null) {
            if (this.location.distanceTo(location) < 1000.0f) {
                this.distanceTextView.setText(String.format("%s - %d %s", new Object[]{formatLocationUpdateDate, Integer.valueOf((int) this.location.distanceTo(location)), LocaleController.getString("MetersAway", C1067R.string.MetersAway)}));
                return;
            } else {
                this.distanceTextView.setText(String.format("%s - %.2f %s", new Object[]{formatLocationUpdateDate, Float.valueOf(this.location.distanceTo(location) / 1000.0f), LocaleController.getString("KMetersAway", C1067R.string.KMetersAway)}));
                return;
            }
        }
    ...
    public void onDraw(Canvas canvas) {
        if (this.currentInfo != null || this.liveLocation != null) {
            ...
            if (i >= currentTime) {
                ...
                if (LocaleController.isRTL) {
                    rectF = this.rect;
                    float dp2 = (float) AndroidUtilities.m26dp(13.0f);
                    if (this.distanceTextView == null) {
                        f2 = 12.0f;
                    }
                    f2 = (float) AndroidUtilities.m26dp(f2);
                    dp = (float) AndroidUtilities.m26dp(43.0f);
                    if (this.distanceTextView == null) {
                        f = 42.0f;
                    }
                    rectF.set(dp2, f2, dp, (float) AndroidUtilities.m26dp(f));
                } else {
                    rectF = this.rect;
                    float measuredWidth = (float) (getMeasuredWidth() - AndroidUtilities.m26dp(43.0f));
                    if (this.distanceTextView == null) {
                        f2 = 12.0f;
                    }
                    f2 = (float) AndroidUtilities.m26dp(f2);
                    dp = (float) (getMeasuredWidth() - AndroidUtilities.m26dp(13.0f));
                    if (this.distanceTextView == null) {
                        f = 42.0f;
                    }
                    rectF.set(measuredWidth, f2, dp, (float) AndroidUtilities.m26dp(f));
                }
                if (this.distanceTextView == null) {
                    i2 = Theme.getColor(Theme.key_dialog_liveLocationProgress);
                } else {
                    i2 = Theme.getColor(Theme.key_location_liveLocationProgress);
                }
                ...
                canvas.drawText(formatLocationLeftTime, this.rect.centerX() - (Theme.chat_livePaint.measureText(formatLocationLeftTime) / 2.0f), (float) AndroidUtilities.m26dp(this.distanceTextView != null ? 37.0f : 31.0f), Theme.chat_livePaint);
            }
        }
    }
}
