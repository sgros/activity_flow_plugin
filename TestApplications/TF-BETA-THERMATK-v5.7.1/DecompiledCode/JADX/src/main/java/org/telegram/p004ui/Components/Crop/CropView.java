...
import org.telegram.p004ui.Components.Crop.CropAreaView.AreaViewListener;
import org.telegram.p004ui.Components.Crop.CropAreaView.GridType;
import org.telegram.p004ui.Components.Crop.CropGestureDetector.CropGestureListener;
...
public class CropView extends FrameLayout implements AreaViewListener, CropGestureListener {
    ...
    class C27951 implements OnPreDrawListener {
        C27951() {
        }
    ...
    private class CropState {
        ...
        /* synthetic */ CropState(CropView cropView, Bitmap bitmap, int i, C27951 c27951) {
            this(bitmap, i);
        }
        ...
        private boolean hasChanges() {
            return Math.abs(this.f582x) > CropView.EPSILON || Math.abs(this.f583y) > CropView.EPSILON || Math.abs(this.scale - this.minimumScale) > CropView.EPSILON || Math.abs(this.rotation) > CropView.EPSILON || Math.abs(this.orientation) > CropView.EPSILON;
        }
    ...
    public void setBitmap(Bitmap bitmap, int i, boolean z, boolean z2) {
        ...
        if (cropState == null || !z2) {
            ...
            this.imageView.getViewTreeObserver().addOnPreDrawListener(new C27951());
        } else {
    ...
    private void fillAreaView(RectF rectF, boolean z) {
        ...
        if (this.state.getScale() * max > MAX_SCALE) {
            access$1100 = MAX_SCALE / this.state.getScale();
            z2 = true;
        } else {
            access$1100 = max;
            z2 = false;
        ...
        ofFloat.addUpdateListener(new C2789-$$Lambda$CropView$u7JJSQis3TQtsCeT54hdvCdMU-Y(this, access$1100, fArr, access$1200, centerY));
    ...
    private void fitContentInBounds(boolean z, boolean z2, boolean z3, boolean z4) {
        if (this.state != null) {
            ...
            if (z3) {
                ...
                if (Math.abs(toRadians - 1.0f) >= EPSILON || Math.abs(access$1300) >= EPSILON || Math.abs(access$1500) >= EPSILON) {
                    this.animating = true;
                    float[] fArr = new float[]{1.0f, 0.0f, 0.0f};
                    ValueAnimator ofFloat = ValueAnimator.ofFloat(new float[]{0.0f, 1.0f});
                    ofFloat.addUpdateListener(new C2790-$$Lambda$CropView$wiih4K5GW50uIgzkO67FnI-u-xc(this, access$1300, fArr, access$1500, toRadians));
                    final boolean z5 = z4;
                    final boolean z6 = z;
                    final boolean z7 = z2;
                    final boolean z8 = z3;
    ...
    public void showAspectRatioDialog() {
        if (this.areaView.getLockAspectRatio() > 0.0f) {
            ...
            r1[0] = LocaleController.getString("CropOriginal", C1067R.string.CropOriginal);
            r1[1] = LocaleController.getString("CropSquare", C1067R.string.CropSquare);
            int i = 2;
            ...
            AlertDialog create = new Builder(getContext()).setItems(r1, new C2787-$$Lambda$CropView$05yvHZ5A2zqkNRsfdUscWVy3MHM(this, r3)).create();
            create.setCanceledOnTouchOutside(true);
            create.setOnCancelListener(new C2788-$$Lambda$CropView$MBImByPOxBcWFdcV4k41dkfucIY(this));
            create.show();
        }
    }
