...
public class CallSwipeView extends View {
    ...
    class C29901 extends AnimatorListenerAdapter {
        private Runnable restarter = new C29891();
        private long startTime;

        /* renamed from: org.telegram.ui.Components.voip.CallSwipeView$1$1 */
        class C29891 implements Runnable {
            C29891() {
            }
        ...
        public void onAnimationEnd(Animator animator) {
            if (System.currentTimeMillis() - this.startTime < animator.getDuration() / 4) {
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m31w("Not repeating animation because previous loop was too fast");
                }
                return;
            }
    ...
    private void init() {
        ...
        this.arrowsPaint.setStrokeWidth((float) AndroidUtilities.m26dp(2.5f));
        this.pullBgPaint = new Paint(1);
        ArrayList arrayList = new ArrayList();
        for (int i = 0; i < this.arrowAlphas.length; i++) {
            ObjectAnimator ofInt = ObjectAnimator.ofInt(new ArrowAnimWrapper(i), "arrowAlpha", new int[]{64, NalUnitUtil.EXTENDED_SAR, 64});
        ...
        this.arrowAnim.addListener(new C29901());
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        canvas.save();
        if (this.dragFromRight) {
            canvas.translate((float) ((getWidth() - getHeight()) - AndroidUtilities.m26dp(18.0f)), (float) (getHeight() / 2));
        } else {
            canvas.translate((float) (getHeight() + AndroidUtilities.m26dp(12.0f)), (float) (getHeight() / 2));
        }
        float abs = Math.abs(this.viewToDrag.getTranslationX());
        for (int i = 0; i < 3; i++) {
            float f = 16.0f;
            float f2 = 1.0f;
            if (abs > ((float) AndroidUtilities.m26dp((float) (i * 16)))) {
                f2 = 1.0f - Math.min(1.0f, Math.max(0.0f, (abs - ((float) (AndroidUtilities.m26dp(16.0f) * i))) / ((float) AndroidUtilities.m26dp(16.0f))));
            }
            ...
            canvas.translate((float) AndroidUtilities.m26dp(f), 0.0f);
        }
        canvas.restore();
        invalidate();
    }
    ...
    private void updateArrowPath() {
        this.arrow.reset();
        int dp = AndroidUtilities.m26dp(6.0f);
        if (this.dragFromRight) {
            float f = (float) dp;
