...
public class CropRotationWheel extends FrameLayout {
    ...
    public CropRotationWheel(Context context) {
        ...
        this.whitePaint.setAlpha(NalUnitUtil.EXTENDED_SAR);
        this.whitePaint.setAntiAlias(true);
        this.bluePaint = new Paint();
        this.bluePaint.setStyle(Style.FILL);
        this.bluePaint.setColor(-11420173);
        this.bluePaint.setAlpha(NalUnitUtil.EXTENDED_SAR);
        this.bluePaint.setAntiAlias(true);
        this.aspectRatioButton = new ImageView(context);
        this.aspectRatioButton.setImageResource(C1067R.C1065drawable.tool_cropfix);
        ...
        this.aspectRatioButton.setOnClickListener(new C2786-$$Lambda$CropRotationWheel$o9DV-6J5Q1lFFifNNTKEl8cNF9w(this));
        this.aspectRatioButton.setContentDescription(LocaleController.getString("AccDescrAspectRatio", C1067R.string.AccDescrAspectRatio));
        addView(this.aspectRatioButton, LayoutHelper.createFrame(70, 64, 19));
        ImageView imageView = new ImageView(context);
        imageView.setImageResource(C1067R.C1065drawable.tool_rotate);
        imageView.setBackgroundDrawable(Theme.createSelectorDrawable(Theme.ACTION_BAR_WHITE_SELECTOR_COLOR));
        imageView.setScaleType(ScaleType.CENTER);
        imageView.setOnClickListener(new C2785-$$Lambda$CropRotationWheel$Is9w1zkokBjEYWq9bkepljuCiac(this));
        imageView.setContentDescription(LocaleController.getString("AccDescrRotate", C1067R.string.AccDescrRotate));
    ...
    public void onMeasure(int i, int i2) {
        super.onMeasure(MeasureSpec.makeMeasureSpec(Math.min(MeasureSpec.getSize(i), AndroidUtilities.m26dp(400.0f)), 1073741824), i2);
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        this.bluePaint.setAlpha(NalUnitUtil.EXTENDED_SAR);
        this.tempRect.left = (float) ((width - AndroidUtilities.m26dp(2.5f)) / 2);
        this.tempRect.top = (float) ((height - AndroidUtilities.m26dp(22.0f)) / 2);
        this.tempRect.right = (float) ((width + AndroidUtilities.m26dp(2.5f)) / 2);
        this.tempRect.bottom = (float) ((height + AndroidUtilities.m26dp(22.0f)) / 2);
        canvas.drawRoundRect(this.tempRect, (float) AndroidUtilities.m26dp(2.0f), (float) AndroidUtilities.m26dp(2.0f), this.bluePaint);
    }

    /* Access modifiers changed, original: protected */
    public void drawLine(Canvas canvas, int i, float f, int i2, int i3, boolean z, Paint paint) {
        int dp = (int) ((((float) i2) / 2.0f) - ((float) AndroidUtilities.m26dp(70.0f)));
        double d = (double) dp;
        ...
        i = Math.min(NalUnitUtil.EXTENDED_SAR, Math.max(0, (int) ((1.0f - (abs * abs)) * 255.0f)));
        if (z) {
            paint = this.bluePaint;
        }
        Paint paint2 = paint;
        paint2.setAlpha(i);
        i = z ? 4 : 2;
        int dp2 = AndroidUtilities.m26dp(z ? 16.0f : 12.0f);
        i /= 2;
