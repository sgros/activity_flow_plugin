...
public class MP3Frame {
    ...
    public boolean isChecksumError() {
        ...
        if (((bArr[5] & NalUnitUtil.EXTENDED_SAR) | ((bArr[4] & NalUnitUtil.EXTENDED_SAR) << 8)) != crc16.getValue()) {
            return true;
        }
        return false;
    }
    ...
    public int getNumberOfFrames() {
        ...
        if (isXingFrame()) {
            ...
            if ((bArr[xingOffset + 7] & 1) != 0) {
                i = (((bArr[xingOffset + 8] & NalUnitUtil.EXTENDED_SAR) << 24) | ((bArr[xingOffset + 9] & NalUnitUtil.EXTENDED_SAR) << 16)) | ((bArr[xingOffset + 10] & NalUnitUtil.EXTENDED_SAR) << 8);
        ...
        if (isVBRIFrame()) {
            ...
            bArr = this.bytes;
            i = (((bArr[xingOffset + 14] & NalUnitUtil.EXTENDED_SAR) << 24) | ((bArr[xingOffset + 15] & NalUnitUtil.EXTENDED_SAR) << 16)) | ((bArr[xingOffset + 16] & NalUnitUtil.EXTENDED_SAR) << 8);
            xingOffset = bArr[xingOffset + 17];
        }
        return -1;
        return (xingOffset & NalUnitUtil.EXTENDED_SAR) | i;
    }
}
