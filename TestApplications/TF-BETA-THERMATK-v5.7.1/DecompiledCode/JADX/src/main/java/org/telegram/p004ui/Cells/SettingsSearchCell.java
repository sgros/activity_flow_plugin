...
public class SettingsSearchCell extends FrameLayout {
    ...
    public void setTextAndValueAndIcon(CharSequence charSequence, String[] strArr, int i, boolean z) {
        ...
        float f = 16.0f;
        layoutParams.leftMargin = AndroidUtilities.m26dp(LocaleController.isRTL ? 16.0f : 71.0f);
        layoutParams.rightMargin = AndroidUtilities.m26dp(LocaleController.isRTL ? 71.0f : 16.0f);
        if (strArr != null) {
            SpannableStringBuilder spannableStringBuilder = new SpannableStringBuilder();
            for (int i2 = 0; i2 < strArr.length; i2++) {
                if (i2 != 0) {
                    spannableStringBuilder.append(" > ");
                    Drawable mutate = getContext().getResources().getDrawable(C1067R.C1065drawable.settings_arrow).mutate();
                    mutate.setBounds(0, 0, mutate.getIntrinsicWidth(), mutate.getIntrinsicHeight());
                    mutate.setColorFilter(new PorterDuffColorFilter(Theme.getColor(Theme.key_windowBackgroundWhiteGrayText2), Mode.MULTIPLY));
            ...
            layoutParams.topMargin = AndroidUtilities.m26dp(21.0f);
        ...
        if (z) {
            ...
            for (i = 0; i < strArr.length; i++) {
                if (i != 0) {
                    ...
                    mutate = getContext().getResources().getDrawable(C1067R.C1065drawable.settings_arrow).mutate();
                    mutate.setBounds(0, 0, mutate.getIntrinsicWidth(), mutate.getIntrinsicHeight());
                    mutate.setColorFilter(new PorterDuffColorFilter(Theme.getColor(Theme.key_windowBackgroundWhiteBlackText), Mode.MULTIPLY));
                    spannableStringBuilder.setSpan(new VerticalImageSpan(mutate), spannableStringBuilder.length() - 2, spannableStringBuilder.length() - 1, 33);
                }
    ...
    public void onDraw(Canvas canvas) {
        if (this.needDivider) {
            canvas.drawLine(LocaleController.isRTL ? 0.0f : (float) AndroidUtilities.m26dp((float) this.left), (float) (getMeasuredHeight() - 1), (float) (getMeasuredWidth() - (LocaleController.isRTL ? AndroidUtilities.m26dp((float) this.left) : 0)), (float) (getMeasuredHeight() - 1), Theme.dividerPaint);
        }
    }
}
