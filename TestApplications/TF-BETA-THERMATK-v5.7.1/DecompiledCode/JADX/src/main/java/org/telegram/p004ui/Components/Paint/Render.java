...
public class Render {
    public static RectF RenderPath(Path path, RenderState renderState) {
        ...
        int i = 0;
        if (length == 1) {
            Render.PaintStamp(path.getPoints()[0], renderState);
        } else {
            ...
            while (i < points.length - 1) {
                ...
                i++;
                Render.PaintSegment(point, points[i], renderState);
            }
        }
        path.remainder = renderState.remainder;
        return Render.Draw(renderState);
    }
    ...
    private static void PaintSegment(Point point, Point point2, RenderState renderState) {
        ...
        float atan2 = Math.abs(renderState2.angle) > 0.0f ? renderState2.angle : (float) Math.atan2(substract.f592y, substract.f591x);
