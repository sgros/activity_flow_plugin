...
public class RecordStatusDrawable extends StatusDrawable {
    ...
    public void draw(Canvas canvas) {
        canvas.save();
        canvas.translate(0.0f, (float) ((getIntrinsicHeight() / 2) + AndroidUtilities.m26dp(this.isChat ? 1.0f : 2.0f)));
        for (int i = 0; i < 4; i++) {
            if (i == 0) {
            ...
            float dp = ((float) (AndroidUtilities.m26dp(4.0f) * i)) + (((float) AndroidUtilities.m26dp(4.0f)) * this.progress);
            float f = -dp;
