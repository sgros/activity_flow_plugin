...
public class LocationActivityAdapter extends BaseLocationAdapter {
    ...
    public void setLiveLocations(ArrayList<LiveLocation> arrayList) {
        ...
        for (int i = 0; i < this.currentLiveLocations.size(); i++) {
            if (((LiveLocation) this.currentLiveLocations.get(i)).f611id == clientUserId) {
    ...
    private void updateCell() {
        ...
        if (this.customLocation != null) {
            sendLocationCell.setText(LocaleController.getString("SendSelectedLocation", C1067R.string.SendSelectedLocation), String.format(Locale.US, "(%f,%f)", new Object[]{Double.valueOf(this.customLocation.getLatitude()), Double.valueOf(this.customLocation.getLongitude())}));
            return;
        }
        String str = "SendLocation";
        if (this.gpsLocation != null) {
            String string = LocaleController.getString(str, C1067R.string.SendLocation);
            Object[] objArr = new Object[1];
            objArr[0] = LocaleController.formatPluralString("Meters", (int) this.gpsLocation.getAccuracy());
            sendLocationCell.setText(string, LocaleController.formatString("AccurateTo", C1067R.string.AccurateTo, objArr));
            return;
        }
        sendLocationCell.setText(LocaleController.getString(str, C1067R.string.SendLocation), LocaleController.getString("Loading", C1067R.string.Loading));
    }
    ...
    public void setPulledUp() {
        if (!this.pulledUp) {
            this.pulledUp = true;
            AndroidUtilities.runOnUIThread(new C2250-$$Lambda$LocationActivityAdapter$kPn2n4u9DAIYsBitfbC-1mB05AA(this));
        }
    }
        ...
        if (itemViewType != 0) {
            ...
            if (itemViewType != 1) {
                ...
                if (itemViewType != 2) {
                    ...
                    ((GraySectionCell) viewHolder.itemView).setText(LocaleController.getString("LiveLocations", C1067R.string.LiveLocations));
                    return;
                } else if (this.pulledUp) {
                    ((GraySectionCell) viewHolder.itemView).setText(LocaleController.getString("NearbyPlaces", C1067R.string.NearbyPlaces));
                    return;
                } else {
                    ((GraySectionCell) viewHolder.itemView).setText(LocaleController.getString("ShowNearbyPlaces", C1067R.string.ShowNearbyPlaces));
                    return;
                }
            }
