...
public class VideoForwardDrawable extends Drawable {
    ...
    public VideoForwardDrawable() {
        ...
        while (true) {
            ...
            if (i < iArr.length / 2) {
                ...
                if (i == 0) {
                    i2 = i * 2;
                    this.path1.moveTo((float) AndroidUtilities.m26dp((float) iArr[i2]), (float) AndroidUtilities.m26dp((float) playPath[i2 + 1]));
                } else {
                    i2 = i * 2;
                    this.path1.lineTo((float) AndroidUtilities.m26dp((float) iArr[i2]), (float) AndroidUtilities.m26dp((float) playPath[i2 + 1]));
                }
                i++;
            } else {
                this.path1.close();
    ...
    public void draw(Canvas canvas) {
        ...
        if (this.leftSide) {
            width -= (bounds.width() / 4) - AndroidUtilities.m26dp(16.0f);
        } else {
            width += (bounds.width() / 4) + AndroidUtilities.m26dp(16.0f);
        }
        canvas.save();
        ...
        canvas.drawCircle((float) (((Math.max(bounds.width(), bounds.height()) / 4) * (this.leftSide ? -1 : 1)) + width), (float) (AndroidUtilities.m26dp(16.0f) + height), (float) (Math.max(bounds.width(), bounds.height()) / 2), this.paint);
        canvas.restore();
        canvas.save();
        if (this.leftSide) {
        ...
        if (f2 <= 0.6f) {
            if (f2 < 0.4f) {
                this.paint.setAlpha(Math.min(NalUnitUtil.EXTENDED_SAR, (int) ((f2 * 255.0f) / 0.2f)));
            } else {
                this.paint.setAlpha((int) ((1.0f - ((f2 - 0.4f) / 0.2f)) * 255.0f));
            }
            canvas.drawPath(this.path1, this.paint);
        }
        canvas.translate((float) AndroidUtilities.m26dp(18.0f), 0.0f);
        float f3 = this.animationProgress;
        if (f3 >= 0.2f && f3 <= 0.8f) {
            f3 -= 0.2f;
            if (f3 < 0.4f) {
                this.paint.setAlpha(Math.min(NalUnitUtil.EXTENDED_SAR, (int) ((f3 * 255.0f) / 0.2f)));
            } else {
    ...
    public int getIntrinsicWidth() {
        return AndroidUtilities.m26dp(32.0f);
    }
