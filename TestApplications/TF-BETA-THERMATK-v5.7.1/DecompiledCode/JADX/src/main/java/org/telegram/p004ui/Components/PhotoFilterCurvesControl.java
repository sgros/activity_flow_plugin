...
import org.telegram.p004ui.Components.PhotoFilterView.CurvesValue;
...
public class PhotoFilterCurvesControl extends View {
    ...
    public PhotoFilterCurvesControl(Context context, CurvesToolValue curvesToolValue) {
        ...
        this.paintDash.setStrokeWidth((float) AndroidUtilities.m26dp(2.0f));
        this.paintDash.setStyle(Style.STROKE);
        this.paintCurve.setColor(-1);
        this.paintCurve.setStrokeWidth((float) AndroidUtilities.m26dp(2.0f));
    ...
    L_0x0078:
        return r3;
        */
        throw new UnsupportedOperationException("Method not decompiled: org.telegram.p004ui.Components.PhotoFilterCurvesControl.onTouchEvent(android.view.MotionEvent):boolean");
    }
    ...
    private void selectSegmentWithPoint(float f) {
        if (this.activeSegment == 0) {
            Rect rect = this.actualArea;
            this.activeSegment = (int) Math.floor((double) (((f - rect.f602x) / (rect.width / 5.0f)) + 1.0f));
        }
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (i3 = 0; i3 < 5; i3++) {
            ...
            canvas.drawText(format, (rect3.f602x + ((f3 - f2) / 2.0f)) + (((float) i3) * f3), (rect3.f603y + rect3.height) - ((float) AndroidUtilities.m26dp(4.0f)), this.textPaint);
        }
        ...
        while (i < interpolateCurve.length / 2) {
            ...
            if (i == 0) {
                ...
                i4 = i * 2;
                path.moveTo(rect4.f602x + (interpolateCurve[i4] * rect4.width), rect4.f603y + ((1.0f - interpolateCurve[i4 + 1]) * rect4.height));
            } else {
                path = this.path;
                rect4 = this.actualArea;
                i4 = i * 2;
                path.lineTo(rect4.f602x + (interpolateCurve[i4] * rect4.width), rect4.f603y + ((1.0f - interpolateCurve[i4 + 1]) * rect4.height));
            }
            i++;
        }
