...
public class DialogRadioCell extends FrameLayout {
    ...
    public void onMeasure(int i, int i2) {
        ...
        this.radioButton.measure(MeasureSpec.makeMeasureSpec(AndroidUtilities.m26dp(22.0f), Integer.MIN_VALUE), MeasureSpec.makeMeasureSpec(AndroidUtilities.m26dp(22.0f), 1073741824));
    ...
    public void onDraw(Canvas canvas) {
        if (this.needDivider) {
            float f = 0.0f;
            float dp = (float) AndroidUtilities.m26dp(LocaleController.isRTL ? 0.0f : 60.0f);
            float height = (float) (getHeight() - 1);
            int measuredWidth = getMeasuredWidth();
            if (LocaleController.isRTL) {
                f = 60.0f;
            }
            canvas.drawLine(dp, height, (float) (measuredWidth - AndroidUtilities.m26dp(f)), (float) (getHeight() - 1), Theme.dividerPaint);
        }
    }
}
