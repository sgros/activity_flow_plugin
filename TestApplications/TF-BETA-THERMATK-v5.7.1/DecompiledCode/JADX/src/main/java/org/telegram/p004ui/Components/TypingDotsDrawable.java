...
public class TypingDotsDrawable extends StatusDrawable {
    ...
    public void draw(Canvas canvas) {
        ...
        int i;
        if (this.isChat) {
            dp = AndroidUtilities.m26dp(8.5f);
            i = getBounds().top;
        } else {
            dp = AndroidUtilities.m26dp(9.3f);
            i = getBounds().top;
        }
        dp += i;
        Theme.chat_statusPaint.setAlpha(NalUnitUtil.EXTENDED_SAR);
        float f = (float) dp;
        canvas.drawCircle((float) AndroidUtilities.m26dp(3.0f), f, this.scales[0] * AndroidUtilities.density, Theme.chat_statusPaint);
        canvas.drawCircle((float) AndroidUtilities.m26dp(9.0f), f, this.scales[1] * AndroidUtilities.density, Theme.chat_statusPaint);
        canvas.drawCircle((float) AndroidUtilities.m26dp(15.0f), f, this.scales[2] * AndroidUtilities.density, Theme.chat_statusPaint);
        checkUpdate();
    }
    ...
    private void checkUpdate() {
        ...
        if (NotificationCenter.getInstance(this.currentAccount).isAnimationInProgress()) {
            AndroidUtilities.runOnUIThread(new C2708-$$Lambda$TypingDotsDrawable$6mZKSEfaAngfDGlsoqdZ2efS_EU(this), 100);
        } else {
            update();
        }
    }
