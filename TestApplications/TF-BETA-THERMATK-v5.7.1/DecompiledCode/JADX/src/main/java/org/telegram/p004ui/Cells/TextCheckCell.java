...
public class TextCheckCell extends FrameLayout {
    ...
    class C23722 extends AnimatorListenerAdapter {
        C23722() {
        }
    ...
    public TextCheckCell(Context context, int i, boolean z) {
        ...
        this.valueTextView.setTextColor(Theme.getColor(z ? Theme.key_dialogIcon : Theme.key_windowBackgroundWhiteGrayText2));
    ...
    public void onMeasure(int i, int i2) {
        if (this.isMultiline) {
            ...
            super.onMeasure(MeasureSpec.makeMeasureSpec(MeasureSpec.getSize(i), 1073741824), MeasureSpec.makeMeasureSpec(AndroidUtilities.m26dp(this.valueTextView.getVisibility() == 0 ? 64.0f : (float) this.height) + this.needDivider, 1073741824));
        }
    }
    ...
    public void setTextAndValueAndCheck(String str, String str2, boolean z, boolean z2, boolean z3) {
        ...
        if (z2) {
            ...
            this.valueTextView.setPadding(0, 0, 0, AndroidUtilities.m26dp(11.0f));
        } else {
            this.valueTextView.setLines(1);
            this.valueTextView.setMaxLines(1);
            this.valueTextView.setSingleLine(true);
    ...
    public void setBackgroundColorAnimated(boolean z, int i) {
        ...
        this.animator.addListener(new C23722());
    ...
    private void setAnimationProgress(float f) {
        ...
        f = Math.max(this.lastTouchX, ((float) getMeasuredWidth()) - this.lastTouchX) + ((float) AndroidUtilities.m26dp(40.0f));
    ...
    public void onDraw(Canvas canvas) {
        if (this.animatedColorBackground != 0) {
            canvas.drawCircle(this.lastTouchX, (float) (getMeasuredHeight() / 2), (Math.max(this.lastTouchX, ((float) getMeasuredWidth()) - this.lastTouchX) + ((float) AndroidUtilities.m26dp(40.0f))) * this.animationProgress, this.animationPaint);
        }
        if (this.needDivider) {
    ...
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo accessibilityNodeInfo) {
        ...
        if (this.checkBox.isChecked()) {
            i = C1067R.string.NotificationsOn;
            str = "NotificationsOn";
        } else {
            i = C1067R.string.NotificationsOff;
