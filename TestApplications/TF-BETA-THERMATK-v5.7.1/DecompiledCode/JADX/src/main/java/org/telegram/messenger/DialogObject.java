...
public class DialogObject {
    ...
    public static void initDialog(Dialog dialog) {
        if (dialog != null && dialog.f440id == 0) {
            if (dialog instanceof TL_dialog) {
                Peer peer = dialog.peer;
                if (peer != null) {
                    int i = peer.user_id;
                    if (i != 0) {
                        dialog.f440id = (long) i;
                    } else {
                        i = peer.chat_id;
                        if (i != 0) {
                            dialog.f440id = (long) (-i);
                        } else {
                            dialog.f440id = (long) (-peer.channel_id);
                        }
                    }
                }
            } else if (dialog instanceof TL_dialogFolder) {
                dialog.f440id = makeFolderDialogId(((TL_dialogFolder) dialog).folder.f485id);
            }
        }
    }
