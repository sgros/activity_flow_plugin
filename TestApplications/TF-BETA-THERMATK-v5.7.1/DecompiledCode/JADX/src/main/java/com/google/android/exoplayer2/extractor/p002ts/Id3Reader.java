...
public final class Id3Reader implements ElementaryStreamReader {
    ...
    public void consume(ParsableByteArray parsableByteArray) {
        if (this.writingSample) {
            ...
            if (i < 10) {
                i = Math.min(bytesLeft, 10 - i);
                System.arraycopy(parsableByteArray.data, parsableByteArray.getPosition(), this.id3Header.data, this.sampleBytesRead, i);
                if (this.sampleBytesRead + i == 10) {
                    ...
                    if (73 == this.id3Header.readUnsignedByte() && 68 == this.id3Header.readUnsignedByte() && 51 == this.id3Header.readUnsignedByte()) {
                        ...
                        Log.m18w("Id3Reader", "Discarding invalid ID3 tag");
                        this.writingSample = false;
                        return;
                    }
                }
