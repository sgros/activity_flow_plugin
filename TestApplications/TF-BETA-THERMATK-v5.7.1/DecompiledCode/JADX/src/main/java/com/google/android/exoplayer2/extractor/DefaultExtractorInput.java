...
public final class DefaultExtractorInput implements ExtractorInput {
    ...
    private void ensureSpaceForPeek(int i) {
        ...
        if (i2 > bArr.length) {
            this.peekBuffer = Arrays.copyOf(this.peekBuffer, Util.constrainValue(bArr.length * 2, MessagesController.UPDATE_MASK_CHECK + i2, i2 + 524288));
        }
    }
    ...
    private void updatePeekBuffer(int i) {
        ...
        if (i2 < obj.length - 524288) {
            obj = new byte[(i2 + MessagesController.UPDATE_MASK_CHECK)];
        }
