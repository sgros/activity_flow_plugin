...
public final class SilenceSkippingAudioProcessor implements AudioProcessor {
    ...
    public void flush() {
        if (isActive()) {
            ...
            durationUsToFrames = this.paddingBuffer.length;
            int i = this.paddingSize;
            if (durationUsToFrames != i) {
    ...
    private void processMaybeSilence(ByteBuffer byteBuffer) {
        ...
        if (findNoisePosition >= limit || position >= length) {
            findNoisePosition = Math.min(position, length);
            byteBuffer.limit(byteBuffer.position() + findNoisePosition);
            byteBuffer.get(this.maybeSilenceBuffer, this.maybeSilenceBufferSize, findNoisePosition);
            this.maybeSilenceBufferSize += findNoisePosition;
            findNoisePosition = this.maybeSilenceBufferSize;
            byte[] bArr2 = this.maybeSilenceBuffer;
            if (findNoisePosition == bArr2.length) {
                if (this.hasOutputNoise) {
                    output(bArr2, this.paddingSize);
                    this.skippedFrames += (long) ((this.maybeSilenceBufferSize - (this.paddingSize * 2)) / this.bytesPerFrame);
                } else {
                    this.skippedFrames += (long) ((findNoisePosition - this.paddingSize) / this.bytesPerFrame);
                }
