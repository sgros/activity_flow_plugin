...
class FloatingActionButtonLollipop extends FloatingActionButtonImpl {
   ...
   void onElevationsChanged(float var1, float var2) {
      if (VERSION.SDK_INT == 21) {
         ...
         var6.add(ObjectAnimator.ofFloat(this.mView, "elevation", new float[]{var1}).setDuration(0L));
         if (VERSION.SDK_INT >= 22 && VERSION.SDK_INT <= 24) {
            var6.add(ObjectAnimator.ofFloat(this.mView, View.TRANSLATION_Z, new float[]{this.mView.getTranslationZ()}).setDuration(100L));
         }

         var6.add(ObjectAnimator.ofFloat(this.mView, View.TRANSLATION_Z, new float[]{0.0F}).setDuration(100L));
         var5.playSequentially((Animator[])var6.toArray(new ObjectAnimator[0]));
         var5.setInterpolator(ANIM_INTERPOLATOR);
         var3.addState(ENABLED_STATE_SET, var5);
         var4 = new AnimatorSet();
