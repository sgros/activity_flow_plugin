...
public final class ColorUtils {
    ...
    public static int calculateMinimumAlpha(@ColorInt int i, @ColorInt int i2, float f) {
        ...
        double d = (double) f;
        if (ColorUtils.calculateContrast(ColorUtils.setAlphaComponent(i, 255), i2) < d) {
            return -1;
        }
        int i4 = 0;
        int i5 = 0;
        while (i4 <= 10 && i3 - i5 > 1) {
            int i6 = (i5 + i3) / 2;
            if (ColorUtils.calculateContrast(ColorUtils.setAlphaComponent(i, i6), i2) < d) {
                i5 = i6;
            } else {
                i3 = i6;
            }
    ...
    public static void XYZToLAB(@FloatRange(from = 0.0d, to = 95.047d) double d, @FloatRange(from = 0.0d, to = 100.0d) double d2, @FloatRange(from = 0.0d, to = 108.883d) double d3, @NonNull double[] dArr) {
        ...
        d = ColorUtils.pivotXyzComponent(d / XYZ_WHITE_REFERENCE_X);
        d2 = ColorUtils.pivotXyzComponent(d2 / XYZ_WHITE_REFERENCE_Y);
        d3 = ColorUtils.pivotXyzComponent(d3 / XYZ_WHITE_REFERENCE_Z);
        dArr[0] = Math.max(Utils.DOUBLE_EPSILON, (116.0d * d2) - 16.0d);
        dArr[1] = 500.0d * (d - d2);
        dArr[2] = 200.0d * (d2 - d3);
    }
    ...
    public static int XYZToColor(@FloatRange(from = 0.0d, to = 95.047d) double d, @FloatRange(from = 0.0d, to = 100.0d) double d2, @FloatRange(from = 0.0d, to = 108.883d) double d3) {
        ...
        return Color.rgb(ColorUtils.constrain((int) Math.round((d4 > 0.0031308d ? (Math.pow(d4, 0.4166666666666667d) * 1.055d) - 0.055d : 12.92d * d4) * 255.0d), 0, 255), ColorUtils.constrain((int) Math.round((d5 > 0.0031308d ? (Math.pow(d5, 0.4166666666666667d) * 1.055d) - 0.055d : 12.92d * d5) * 255.0d), 0, 255), ColorUtils.constrain((int) Math.round(255.0d * (d6 > 0.0031308d ? (1.055d * Math.pow(d6, 0.4166666666666667d)) - 0.055d : 12.92d * d6)), 0, 255));
    }
