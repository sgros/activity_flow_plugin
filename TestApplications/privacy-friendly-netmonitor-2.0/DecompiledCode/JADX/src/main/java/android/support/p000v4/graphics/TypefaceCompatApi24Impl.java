...
class TypefaceCompatApi24Impl extends TypefaceCompatBaseImpl {
    ...
    public Typeface createFromFontInfo(Context context, @Nullable CancellationSignal cancellationSignal, @NonNull FontInfo[] fontInfoArr, int i) {
        Object newFamily = TypefaceCompatApi24Impl.newFamily();
        ...
        for (FontInfo fontInfo : fontInfoArr) {
            ...
            if (!TypefaceCompatApi24Impl.addFontWeightStyle(newFamily, byteBuffer, fontInfo.getTtcIndex(), fontInfo.getWeight(), fontInfo.isItalic())) {
                return null;
            }
        }
        return TypefaceCompatApi24Impl.createFromFamiliesWithDefault(newFamily);
    }
    ...
    public Typeface createFromFontFamilyFilesResourceEntry(Context context, FontFamilyFilesResourceEntry fontFamilyFilesResourceEntry, Resources resources, int i) {
        ...
        for (FontFileResourceEntry fontFileResourceEntry : fontFamilyFilesResourceEntry.getEntries()) {
            if (!TypefaceCompatApi24Impl.addFontWeightStyle(newFamily, TypefaceCompatUtil.copyToDirectBuffer(context, resources, fontFileResourceEntry.getResourceId()), 0, fontFileResourceEntry.getWeight(), fontFileResourceEntry.isItalic())) {
                return null;
            }
        }
        return TypefaceCompatApi24Impl.createFromFamiliesWithDefault(newFamily);
