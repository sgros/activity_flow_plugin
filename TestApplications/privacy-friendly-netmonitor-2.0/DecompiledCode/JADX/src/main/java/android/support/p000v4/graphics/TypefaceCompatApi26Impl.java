...
public class TypefaceCompatApi26Impl extends TypefaceCompatApi21Impl {
    ...
    public Typeface createFromFontFamilyFilesResourceEntry(Context context, FontFamilyFilesResourceEntry fontFamilyFilesResourceEntry, Resources resources, int i) {
        ...
        Object newFamily = TypefaceCompatApi26Impl.newFamily();
        ...
        while (i2 < i) {
            ...
            if (TypefaceCompatApi26Impl.addFontFromAssetManager(context, newFamily, fontFileResourceEntry.getFileName(), 0, fontFileResourceEntry.getWeight(), fontFileResourceEntry.isItalic())) {
                i2++;
            } else {
        ...
        if (TypefaceCompatApi26Impl.freeze(newFamily)) {
            return TypefaceCompatApi26Impl.createFromFamiliesWithDefault(newFamily);
        }
        return null;
    }
    ...
    public Typeface createFromFontInfo(Context context, @Nullable CancellationSignal cancellationSignal, @NonNull FontInfo[] fontInfoArr, int i) {
        ...
        if (TypefaceCompatApi26Impl.isFontFamilyPrivateAPIAvailable()) {
            ...
            while (i < length) {
                ...
                if (byteBuffer != null) {
                    if (TypefaceCompatApi26Impl.addFontFromBuffer(newFamily, byteBuffer, fontInfo.getTtcIndex(), fontInfo.getWeight(), fontInfo.isItalic())) {
                        i2 = 1;
                    } else {
                        TypefaceCompatApi26Impl.abortCreation(newFamily);
                        return null;
            ...
            if (i2 == 0) {
                TypefaceCompatApi26Impl.abortCreation(newFamily);
                return null;
            } else if (TypefaceCompatApi26Impl.freeze(newFamily)) {
                return TypefaceCompatApi26Impl.createFromFamiliesWithDefault(newFamily);
            } else {
                return null;
            }
    ...
    public Typeface createFromResourcesFontFile(Context context, Resources resources, int i, String str, int i2) {
        ...
        Object newFamily = TypefaceCompatApi26Impl.newFamily();
        if (!TypefaceCompatApi26Impl.addFontFromAssetManager(context, newFamily, str, 0, -1, -1)) {
            TypefaceCompatApi26Impl.abortCreation(newFamily);
            return null;
        } else if (TypefaceCompatApi26Impl.freeze(newFamily)) {
            return TypefaceCompatApi26Impl.createFromFamiliesWithDefault(newFamily);
