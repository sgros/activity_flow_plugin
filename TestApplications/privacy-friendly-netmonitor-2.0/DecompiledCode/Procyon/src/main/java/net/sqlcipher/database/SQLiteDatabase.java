...
public class SQLiteDatabase extends SQLiteClosable
{
    ...
    public void execSQL(String compileStatement, final Object[] array) throws SQLException {
        ...
        final SQLiteProgram sqLiteProgram = null;
        SQLiteProgram sqLiteProgram2 = null;
        Label_0134: {
            try {
                try {
            ...
            catch (SQLiteDatabaseCorruptException sqLiteProgram2) {}
        }
        this.onCorruption();
        throw sqLiteProgram2;
        if (sqLiteProgram2 != null) {
            sqLiteProgram2.close();
        }
        this.unlock();
    ...
    public int updateWithOnConflict(String s, ContentValues iterator, String compileStatement, final String[] ex, int lastChangeCount) {
        if (iterator == null || ((ContentValues)iterator).size() == 0) {
        ...
        sb.append(s);
        sb.append(" SET ");
        final Set valueSet = ((ContentValues)iterator).valueSet();
        final Iterator<Map.Entry<String, V>> iterator2 = valueSet.iterator();
        ...
        try {
            try {
                ...
                try {
                    final int size = valueSet.size();
                    iterator = (SQLiteDatabaseCorruptException)valueSet.iterator();
                    final int n = 0;
                    lastChangeCount = 1;
                    for (int i = 0; i < size; ++i) {
                        DatabaseUtils.bindObjectToProgram((SQLiteProgram)compileStatement, lastChangeCount, ((Iterator<Map.Entry<K, Object>>)iterator).next().getValue());
                        ++lastChangeCount;
                    }
                    if (ex != null) {
                        for (int length = ex.length, j = n; j < length; ++j) {
                            ((SQLiteProgram)compileStatement).bindString(lastChangeCount, ex[j]);
                            ++lastChangeCount;
                        }
                    }
                    ((SQLiteStatement)compileStatement).execute();
                    ...
                    if (Log.isLoggable("Database", 2)) {
                        iterator = (SQLiteDatabaseCorruptException)new StringBuilder();
                        ((StringBuilder)iterator).append("Updated ");
                        ((StringBuilder)iterator).append(lastChangeCount);
                ...
                catch (SQLiteDatabaseCorruptException iterator) {}
                finally {
                    iterator = compileStatement;
                }
            }
            finally {}
        }
