...
public class ItemTouchHelper extends ItemDecoration implements OnChildAttachStateChangeListener
{
    ...
    boolean scrollIfNecessary() {
        ...
        Label_0318: {
            if (layoutManager.canScrollVertically()) {
                ...
                n5 = n4 - this.mTmpRect.top - this.mRecyclerView.getPaddingTop();
                if (this.mDy < 0.0f && n5 < 0) {
                    break Label_0318;
                }
                if (this.mDy > 0.0f) {
                    n5 = n4 + this.mSelected.itemView.getHeight() + this.mTmpRect.bottom - (this.mRecyclerView.getHeight() - this.mRecyclerView.getPaddingBottom());
                    if (n5 > 0) {
                        break Label_0318;
                    }
                }
        ...
        int interpolateOutOfBoundsScroll = n3;
        if (n3 != 0) {
            interpolateOutOfBoundsScroll = this.mCallback.interpolateOutOfBoundsScroll(this.mRecyclerView, this.mSelected.itemView.getWidth(), n3, this.mRecyclerView.getWidth(), n);
        }
    ...
    public abstract static class Callback
    {
        ...
        public int interpolateOutOfBoundsScroll(final RecyclerView recyclerView, int n, final int a, int n2, final long n3) {
            ...
            float n5 = 1.0f;
            n = (int)(n2 * maxDragScroll * Callback.sDragViewScrollCapInterpolator.getInterpolation(Math.min(1.0f, n4 * 1.0f / n)));
            if (n3 <= 2000L) {
                n5 = n3 / 2000.0f;
            }
