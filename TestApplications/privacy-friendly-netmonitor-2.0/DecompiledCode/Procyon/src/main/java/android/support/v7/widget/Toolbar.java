...
public class Toolbar extends ViewGroup
{
    ...
    protected void onLayout(final boolean b, int n, int i, int a, int min) {
        ...
        if (this.shouldLayout(this.mExpandedActionView)) {
            if (b2) {
                n = this.layoutChildRight(this.mExpandedActionView, a, mTempMargins, min);
                layoutChildLeft2 = i;
            }
            else {
                layoutChildLeft2 = this.layoutChildLeft(this.mExpandedActionView, i, mTempMargins, min);
                n = a;
            }
        }
        a = layoutChildLeft2;
        ...
        if (this.shouldLayout((View)this.mLogoView)) {
            if (b2) {
                i = this.layoutChildRight((View)this.mLogoView, n, mTempMargins, min);
                a = layoutChildLeft2;
            }
            else {
                a = this.layoutChildLeft((View)this.mLogoView, layoutChildLeft2, mTempMargins, min);
                i = n;
            }
        }
    ...
    protected void onMeasure(int resolveSizeAndState, final int n) {
        ...
        while (i < childCount) {
            ...
            if (((LayoutParams)child.getLayoutParams()).mViewType == 0) {
                if (!this.shouldLayout(child)) {
                    ...
                    n14 = n12 + this.measureChildCollapseMargins(child, resolveSizeAndState, n12, n, 0, mTempMargins);
                    max4 = Math.max(n13, child.getMeasuredHeight() + this.getVerticalMargins(child));
        ...
        if (this.shouldLayout((View)this.mSubtitleTextView)) {
            max5 = Math.max(max5, this.measureChildCollapseMargins((View)this.mSubtitleTextView, resolveSizeAndState, n12 + n16, n, b3 + n15, mTempMargins));
        ...
        final int max6 = Math.max(n13, b3);
        final int paddingLeft = this.getPaddingLeft();
        final int paddingRight = this.getPaddingRight();
        final int paddingTop = this.getPaddingTop();
        final int paddingBottom = this.getPaddingBottom();
