...
public class ActionMenuView extends LinearLayoutCompat implements ItemInvoker, MenuView
{
    ...
    @Override
    protected void onLayout(final boolean b, int i, int n, int j, int measuredWidth) {
        if (!this.mFormatItems) {
            super.onLayout(b, i, n, j, measuredWidth);
            return;
        }
        final int childCount = this.getChildCount();
        final int n2 = (measuredWidth - n) / 2;
        ...
        while (j < childCount) {
            ...
            if (child.getVisibility() != 8) {
                final LayoutParams layoutParams = (LayoutParams)child.getLayoutParams();
                if (layoutParams.isOverflowButton) {
                    final int n4 = measuredWidth = child.getMeasuredWidth();
                    if (this.hasSupportDividerBeforeChildAt(j)) {
                        measuredWidth = n4 + dividerWidth;
                    }
        ...
        if (childCount == 1 && measuredWidth == 0) {
            final View child2 = this.getChildAt(0);
            n = child2.getMeasuredWidth();
            i = child2.getMeasuredHeight();
            j = n3 / 2 - n / 2;
            measuredWidth = n2 - i / 2;
            child2.layout(j, measuredWidth, n + j, i + measuredWidth);
            return;
        }
        n -= (measuredWidth ^ 0x1);
        if (n > 0) {
        ...
        if (layoutRtl) {
            ...
            for (i = j; i < childCount; ++i, n = j) {
                ...
                if (child3.getVisibility() != 8) {
                    if (layoutParams2.isOverflowButton) {
                        ...
                        measuredWidth = n - layoutParams2.rightMargin;
                        j = child3.getMeasuredWidth();
                        measuredHeight2 = child3.getMeasuredHeight();
                        n = n2 - measuredHeight2 / 2;
                        child3.layout(measuredWidth - j, n, measuredWidth, measuredHeight2 + n);
                        j = measuredWidth - (j + layoutParams2.leftMargin + max);
                    }
                }
            }
        }
            ...
            int measuredWidth2;
            for (i = measuredWidth; i < childCount; ++i, n = j) {
                ...
                if (child4.getVisibility() != 8) {
                    if (layoutParams3.isOverflowButton) {
                        ...
                        j = n2 - n / 2;
                        child4.layout(measuredWidth, j, measuredWidth + measuredWidth2, n + j);
                        j = measuredWidth + (measuredWidth2 + layoutParams3.rightMargin + max);
                    }
                }
            }
        }
