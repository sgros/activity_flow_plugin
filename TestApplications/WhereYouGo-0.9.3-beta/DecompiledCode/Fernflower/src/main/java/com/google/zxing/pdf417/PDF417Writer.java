      ...
      if (var2 != BarcodeFormat.PDF_417) {
         ...
         if (var5 != null) {
            if (var5.containsKey(EncodeHintType.PDF417_COMPACT)) {
               var6.setCompact(Boolean.valueOf(var5.get(EncodeHintType.PDF417_COMPACT).toString()));
            }

            if (var5.containsKey(EncodeHintType.PDF417_COMPACTION)) {
               var6.setCompaction(Compaction.valueOf(var5.get(EncodeHintType.PDF417_COMPACTION).toString()));
            }

            if (var5.containsKey(EncodeHintType.PDF417_DIMENSIONS)) {
               Dimensions var11 = (Dimensions)var5.get(EncodeHintType.PDF417_DIMENSIONS);
               var6.setDimensions(var11.getMaxCols(), var11.getMinCols(), var11.getMaxRows(), var11.getMinRows());
            }
            ...
            if (var5.containsKey(EncodeHintType.ERROR_CORRECTION)) {
               var8 = Integer.parseInt(var5.get(EncodeHintType.ERROR_CORRECTION).toString());
            }

            var9 = var8;
            var10 = var7;
            if (var5.containsKey(EncodeHintType.CHARACTER_SET)) {
               var6.setEncoding(Charset.forName(var5.get(EncodeHintType.CHARACTER_SET).toString()));
               var10 = var7;
               var9 = var8;
            }
         }

         return bitMatrixFromEncoder(var6, var1, var9, var3, var4, var10);
      }
   }
}
