...
public final class ErrorCorrection {
   ...
   private ModulusPoly[] runEuclideanAlgorithm(ModulusPoly var1, ModulusPoly var2, int var3) throws ChecksumException {
      ...
      while(true) {
         ...
         int var10;
         for(int var9 = this.field.inverse(var8); var2.getDegree() >= var5.getDegree() && !var2.isZero(); var2 = var2.subtract(var5.multiplyByMonomial(var10, var8))) {
            var10 = var2.getDegree() - var5.getDegree();
            var8 = this.field.multiply(var2.getCoefficient(var2.getDegree()), var9);
            var1 = var1.add(this.field.buildMonomial(var10, var8));
         }
   ...
   public int decode(int[] var1, int var2, int[] var3) throws ChecksumException {
      ...
      if (!var6) {
         ...
         var3 = this.findErrorLocations(var4);
         int[] var12 = this.findErrorMagnitudes(var11, var4, var3);

         for(var2 = 0; var2 < var3.length; ++var2) {
            var7 = var1.length - 1 - this.field.log(var3[var2]);
            if (var7 < 0) {
               throw ChecksumException.getChecksumInstance();
            }

            var1[var7] = this.field.subtract(var1[var7], var12[var2]);
         }
