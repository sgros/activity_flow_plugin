   ...
   private List processWays(QueryParameters var1, int var2) {
      ...
      while(true) {
         ...
         label115: {
            ...
            while(true) {
               if (var17 == 0) {
                  byte var9 = this.readBuffer.readByte();
                  boolean var18;
                  if ((var9 & 128) != 0) {
                     var18 = true;
                  } else {
                  ...
                  if (var18) {
                     var5.add(new Tag("name", this.readBuffer.readUTF8EncodedString()));
                  }

                  if (var16) {
                     var5.add(new Tag("addr:housenumber", this.readBuffer.readUTF8EncodedString()));
                  }

                  if (var10) {
                     var5.add(new Tag("ref", this.readBuffer.readUTF8EncodedString()));
                  }

                  GeoPoint var14 = this.readOptionalLabelPosition(var11);
                  var6 = this.readOptionalWayDataBlocksByte(var12);
                  if (var6 < 1) {
                     LOGGER.warning("invalid number of way data blocks: " + var6);
                     this.logDebugSignatures();
                     var5 = null;
                     return var5;
                  }
                  ...
                  while(true) {
                     ...
                     GeoPoint[][] var15 = this.processWayDataBlock(var13);
                     if (var15 == null) {
                        var5 = null;
                        return var5;
                     }

                     var3.add(new Way(var8, var5, var15, var14));
                     ++var7;
                  }
               }

               var6 = this.readBuffer.readUnsignedInt();
               if (var6 < 0 || var6 >= var4.length) {
                  LOGGER.warning("invalid way tag ID: " + var6);
                  this.logDebugSignatures();
                  var5 = null;
                  return var5;
               }
