...
public final class Cache implements Closeable, Flushable {
    ...
    class C03262 implements Iterator<String> {
        boolean canRemove;
        final Iterator<Snapshot> delegate = Cache.this.cache.snapshots();
        String nextUrl;

        C03262() throws IOException {
        }
        ...
        private void writeCertList(BufferedSink sink, List<Certificate> certificates) throws IOException {
            try {
                ...
                for (int i = 0; i < size; i++) {
                    sink.writeUtf8(ByteString.m31of(((Certificate) certificates.get(i)).getEncoded()).base64()).writeByte(10);
                }
    ...
    class C03271 implements InternalCache {
        C03271() {
        }
