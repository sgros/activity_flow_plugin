...
final class SegmentedByteString extends ByteString
{
    ...
    public boolean equals(final Object o) {
        boolean b = true;
        if (o != this) {
            b = (o instanceof ByteString && ((ByteString)o).size() == this.size() && this.rangeEquals(0, (ByteString)o, 0, this.size()));
        }
        return b;
    }
        ...
        if (n >= 0) {
            if (n > this.size() - i) {
                ...
                for (n = segment; i > 0; i -= min, ++n) {
                    ...
                    n5 = this.directory[this.segments.length + n];
                    b2 = b;
                    if (!byteString.rangeEquals(n2, this.segments[n], n3 - n4 + n5, min)) {
                        return b2;
                    }
                    n3 += min;
                    n2 += min;
                ...
                if (n2 >= 0) {
                    if (n2 > array.length - i) {
                        ...
                        for (n = segment; i > 0; i -= min, ++n) {
                            ...
                            n5 = this.directory[this.segments.length + n];
                            b2 = b;
                            if (!Util.arrayRangeEquals(this.segments[n], n3 - n4 + n5, array, n2, min)) {
                                return b2;
                            }
                            n3 += min;
                            n2 += min;
