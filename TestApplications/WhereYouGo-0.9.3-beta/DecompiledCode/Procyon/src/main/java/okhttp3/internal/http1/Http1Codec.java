...
public final class Http1Codec implements HttpCodec
{
    ...
    private Source getTransferStream(final Response response) throws IOException {
        ...
        if (!HttpHeaders.hasBody(response)) {
            source = this.newFixedLengthSource(0L);
        }
        else if ("chunked".equalsIgnoreCase(response.header("Transfer-Encoding"))) {
            source = this.newChunkedSource(response.request().url());
        }
        else {
            final long contentLength = HttpHeaders.contentLength(response);
            if (contentLength != -1L) {
                source = this.newFixedLengthSource(contentLength);
            }
            else {
                source = this.newUnknownLengthSource();
            }
        }
        return source;
    }
    ...
    public Sink createRequestBody(final Request request, final long n) {
        ...
        if ("chunked".equalsIgnoreCase(request.header("Transfer-Encoding"))) {
            ...
            sink = this.newFixedLengthSink(n);
        }
        return sink;
    }
    ...
    public Response.Builder readResponseHeaders(final boolean b) throws IOException {
        ...
        try {
            ...
            Object headers = new Response.Builder().protocol(parse.protocol).code(parse.code).message(parse.message).headers(this.readHeaders());
            if (b && parse.code == 100) {
                headers = null;
            }
            else {
