...
public final class RouteSelector
{
    ...
    private void resetNextInetSocketAddress(final Proxy proxy) throws IOException {
        ...
        if (port < 1 || port > 65535) {
            throw new SocketException("No route to " + s + ":" + port + "; port is out of range");
        }
        if (proxy.type() == Proxy.Type.SOCKS) {
            this.inetSocketAddresses.add(InetSocketAddress.createUnresolved(s, port));
        }
        else {
    ...
    public Route next() throws IOException {
        ...
        Route route;
        final Route route2 = route = new Route(this.address, this.lastProxy, this.lastInetSocketAddress);
