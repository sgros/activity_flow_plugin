...
public abstract class ResultParser
{
    ...
    static String[] matchPrefixedField(final String str, final String s, final char ch, final boolean b) {
        ...
        while (i < s.length()) {
            ...
            while (true) {
                final int fromIndex = i = endIndex;
                list = list2;
                if (n == 0) {
                    break;
                }
                endIndex = s.indexOf(ch, fromIndex);
                if (endIndex < 0) {
                    endIndex = s.length();
                    n = 0;
                }
                else if (countPrecedingBackslashes(s, endIndex) % 2 != 0) {
                    ++endIndex;
                }
                else {
                    List<String> list3;
                    ...
                    String s3;
                    final String s2 = s3 = unescapeBackslash(s.substring(beginIndex, endIndex));
                    if (b) {
                        s3 = s2.trim();
                    }
                    if (!s3.isEmpty()) {
