...
public class Detector
{
    ...
    protected final AlignmentPattern findAlignmentInRegion(final float n, int max, int min, final float n2) throws NotFoundException {
        final int n3 = (int)(n2 * n);
        final int max2 = Math.max(0, max - n3);
        final int min2 = Math.min(this.image.getWidth() - 1, max + n3);
        if (min2 - max2 < n * 3.0f) {
            throw NotFoundException.getNotFoundInstance();
        }
        max = Math.max(0, min - n3);
        min = Math.min(this.image.getHeight() - 1, min + n3);
        if (min - max < n * 3.0f) {
            throw NotFoundException.getNotFoundInstance();
        }
        return new AlignmentPatternFinder(this.image, max2, max, min2 - max2, min - max, n, this.resultPointCallback).find();
    }
