...
public final class ErrorCorrection
{
    ...
    private int[] findErrorMagnitudes(final ModulusPoly modulusPoly, ModulusPoly modulusPoly2, final int[] array) {
        ...
        modulusPoly2 = new ModulusPoly(this.field, array2);
        final int length = array.length;
        final int[] array3 = new int[length];
        for (int j = 0; j < length; ++j) {
            final int inverse = this.field.inverse(array[j]);
            array3[j] = this.field.multiply(this.field.subtract(0, modulusPoly.evaluateAt(inverse)), this.field.inverse(modulusPoly2.evaluateAt(inverse)));
        }
        return array3;
    }
    ...
    public int decode(final int[] array, int i, int[] errorLocations) throws ChecksumException {
        ...
        if (!b) {
            ...
            if (errorLocations != null) {
                for (int length = errorLocations.length, k = 0; k < length; ++k) {
                    modulusPoly2 = modulusPoly2.multiply(new ModulusPoly(this.field, new int[] { this.field.subtract(0, this.field.exp(array.length - 1 - errorLocations[k])), 1 }));
                }
            }
            ...
            errorLocations = this.findErrorLocations(modulusPoly3);
            final int[] errorMagnitudes = this.findErrorMagnitudes(modulusPoly4, modulusPoly3, errorLocations);
            int n;
