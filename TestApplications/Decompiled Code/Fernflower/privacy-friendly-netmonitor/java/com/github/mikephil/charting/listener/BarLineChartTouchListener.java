   ...
   public boolean onTouch(View var1, MotionEvent var2) {
      ...
      if (!((BarLineChartBase)this.mChart).isDragEnabled() && !((BarLineChartBase)this.mChart).isScaleXEnabled() && !((BarLineChartBase)this.mChart).isScaleYEnabled()) {
         ...
         case 1:
            ...
            var8 = var9.getYVelocity(var12);
            var7 = var9.getXVelocity(var12);
            if ((Math.abs(var7) > (float)Utils.getMinimumFlingVelocity() || Math.abs(var8) > (float)Utils.getMinimumFlingVelocity()) && this.mTouchMode == 1 && ((BarLineChartBase)this.mChart).isDragDecelerationEnabled()) {
               this.stopDeceleration();
               this.mDecelerationLastTime = AnimationUtils.currentAnimationTimeMillis();
               this.mDecelerationCurrentPoint.x = var2.getX();
               this.mDecelerationCurrentPoint.y = var2.getY();
               this.mDecelerationVelocity.x = var7;
               this.mDecelerationVelocity.y = var8;
               Utils.postInvalidateOnAnimation(this.mChart);
            }
         ...
         case 2:
            if (this.mTouchMode == 1) {
               ...
               if (this.mTouchMode == 0 && Math.abs(distance(var2.getX(), this.mTouchStartPoint.x, var2.getY(), this.mTouchStartPoint.y)) > this.mDragTriggerDist && ((BarLineChartBase)this.mChart).isDragEnabled()) {
                  ...
                  if (var11) {
                     var8 = Math.abs(var2.getX() - this.mTouchStartPoint.x);
                     var7 = Math.abs(var2.getY() - this.mTouchStartPoint.y);
                     if ((((BarLineChartBase)this.mChart).isDragXEnabled() || var7 >= var8) && (((BarLineChartBase)this.mChart).isDragYEnabled() || var7 <= var8)) {
                        this.mLastGesture = ChartTouchListener.ChartGesture.DRAG;
                        this.mTouchMode = 1;
                     }
