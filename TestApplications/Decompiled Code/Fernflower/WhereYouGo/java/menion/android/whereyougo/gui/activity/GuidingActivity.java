         ...
         public void run() {
            ...
            if (var1 != null && var1.isLocated() && var1.isVisible() && A.getGuidingContent() != null) {
               ...
               if (A.getGuidingContent().getTargetLocation() == null) {
                  ...
                  var2 = new Location(A.getGuidingContent().getTargetLocation());
               }

               if (var1 instanceof Zone) {
                  Zone var3 = (Zone)DetailsActivity.et;
                  if (Preferences.GUIDING_ZONE_NAVIGATION_POINT == 1) {
                     var2.setLatitude(var3.nearestPoint.latitude);
                     var2.setLongitude(var3.nearestPoint.longitude);
                  } else if (var3.position != null) {
                     var2.setLatitude(var3.position.latitude);
                     var2.setLongitude(var3.position.longitude);
                  } else {
                     var2.setLatitude(var3.bbCenter.latitude);
                     var2.setLongitude(var3.bbCenter.longitude);
                  }
               } else {
                  var2.setLatitude(var1.position.latitude);
                  var2.setLongitude(var1.position.longitude);
               }

               A.getGuidingContent().onLocationChanged(var2);
            }
