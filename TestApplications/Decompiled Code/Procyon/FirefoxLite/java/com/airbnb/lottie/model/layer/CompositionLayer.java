...
public class CompositionLayer extends BaseLayer
{
        ...
        while (true) {
            ...
            if (forModel != null) {
                ...
                if (baseLayer != null) {
                    ...
                    switch (CompositionLayer$1.$SwitchMap$com$airbnb$lottie$model$layer$Layer$MatteType[layer2.getMatteType().ordinal()]) {
                        case 1:
                        case 2: {
                            baseLayer = forModel;
                            break;
    ...
    public void getBounds(final RectF rectF, final Matrix matrix) {
        ...
        for (int i = this.layers.size() - 1; i >= 0; --i) {
            this.layers.get(i).getBounds(this.rect, this.boundsMatrix);
            if (rectF.isEmpty()) {
                rectF.set(this.rect);
            }
            else {
    ...
    public void setProgress(float progress) {
        ...
        if (this.timeRemapping != null) {
            progress = this.lottieDrawable.getComposition().getDuration();
            progress = (long)(this.timeRemapping.getValue() * 1000.0f) / progress;
        }
        float n = progress;
        if (this.layerModel.getTimeStretch() != 0.0f) {
            n = progress / this.layerModel.getTimeStretch();
        }
