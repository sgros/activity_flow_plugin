...
public abstract class Visibility extends Transition
{
    ...
    public Animator onDisappear(final ViewGroup viewGroup, final TransitionValues transitionValues, int n, final TransitionValues transitionValues2, int n2) {
        ...
        View copyViewImage = null;
        Label_0235: {
            Label_0097: {
                if (view2 != null && view2.getParent() != null) {
                    if (n2 != 4) {
                        if ((copyViewImage = view) != view2) {
                            break Label_0097;
                        }
                    }
                    copyViewImage = null;
                    ...
                    if (view != null) {
                        ...
                        if (view.getParent() instanceof View) {
                            ...
                            if (!this.getVisibilityChangeInfo(this.getTransitionValues(view3, true), this.getMatchedTransitionValues(view3, true)).mVisibilityChange) {
                                copyViewImage = TransitionUtils.copyViewImage(viewGroup, view, view3);
                                break Label_0097;
                            }
        ...
        if (copyViewImage != null && transitionValues != null) {
            final int[] array = transitionValues.values.get("android:visibility:screenLocation");
            n = array[0];
            n2 = array[1];
            final int[] array2 = new int[2];
            viewGroup.getLocationOnScreen(array2);
            copyViewImage.offsetLeftAndRight(n - array2[0] - copyViewImage.getLeft());
            copyViewImage.offsetTopAndBottom(n2 - array2[1] - copyViewImage.getTop());
            final ViewGroupOverlayImpl overlay = ViewGroupUtils.getOverlay(viewGroup);
            overlay.add(copyViewImage);
            final Animator onDisappear = this.onDisappear(viewGroup, copyViewImage, transitionValues, transitionValues2);
            if (onDisappear == null) {
                overlay.remove(copyViewImage);
            }
            else {
