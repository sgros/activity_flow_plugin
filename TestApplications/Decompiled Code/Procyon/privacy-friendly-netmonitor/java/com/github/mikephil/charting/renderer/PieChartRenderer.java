...
public class PieChartRenderer extends DataRenderer
{
    ...
    protected float calculateMinimumRadiusForSpacedSlice(final MPPointF mpPointF, final float n, final float n2, final float n3, final float n4, float y, float x) {
        final float n5 = x / 2.0f;
        final float x2 = mpPointF.x;
        final double n6 = (y + x) * 0.017453292f;
        final float n7 = x2 + (float)Math.cos(n6) * n;
        final float n8 = mpPointF.y + (float)Math.sin(n6) * n;
        x = mpPointF.x;
        final double n9 = (y + n5) * 0.017453292f;
        final float n10 = (float)Math.cos(n9);
        y = mpPointF.y;
        return (float)(n - (float)(Math.sqrt(Math.pow(n7 - n3, 2.0) + Math.pow(n8 - n4, 2.0)) / 2.0 * Math.tan(0.017453292519943295 * ((180.0 - n2) / 2.0))) - Math.sqrt(Math.pow(x + n10 * n - (n7 + n3) / 2.0f, 2.0) + Math.pow(y + (float)Math.sin(n9) * n - (n8 + n4) / 2.0f, 2.0)));
    }
    ...
    public void drawValues(final Canvas canvas) {
        ...
        while (i < dataSets.size()) {
            ...
            if (!drawValuesEnabled && !drawEntryLabelsEnabled) {
                ...
                float[] array4;
                MPPointF mpPointF7;
                for (int j = n10; j < entryCount; ++j, array4 = absoluteAngles, mpPointF7 = mpPointF4, pieData = pieData3, mpPointF2 = mpPointF7, absoluteAngles = array4) {
                    ...
                    if (n4 == 0) {
                        ...
                        n12 = absoluteAngles[n4 - 1] * phaseX;
                    }
                    final float n13 = n5 + (n12 + (array3[n4] - sliceSpace / (0.017453292f * n3) / 2.0f) / 2.0f) * phaseY;
                    float y;
