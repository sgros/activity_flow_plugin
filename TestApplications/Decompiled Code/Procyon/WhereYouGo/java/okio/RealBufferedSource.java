...
final class RealBufferedSource implements BufferedSource
{
        ...
        return new InputStream() {
            ...
            public int read() throws IOException {
                ...
                if (RealBufferedSource.this.buffer.size == 0L && RealBufferedSource.this.source.read(RealBufferedSource.this.buffer, 8192L) == -1L) {
                    ...
                    n = (RealBufferedSource.this.buffer.readByte() & 0xFF);
                }
                return n;
            }
            ...
            public int read(final byte[] array, int read, final int n) throws IOException {
                ...
                Util.checkOffsetAndCount(array.length, read, n);
                if (RealBufferedSource.this.buffer.size == 0L && RealBufferedSource.this.source.read(RealBufferedSource.this.buffer, 8192L) == -1L) {
                    read = -1;
                }
                else {
                    read = RealBufferedSource.this.buffer.read(array, read, n);
                }
                return read;
            }
    ...
    public int read(final byte[] array, int read, int n) throws IOException {
        ...
        if (this.buffer.size == 0L && this.source.read(this.buffer, 8192L) == -1L) {
            ...
            n = (int)Math.min(n, this.buffer.size);
            read = this.buffer.read(array, read, n);
        }
        return read;
    }
    ...
    public long read(final Buffer buffer, long n) throws IOException {
        ...
        if (this.buffer.size == 0L && this.source.read(this.buffer, 8192L) == -1L) {
            ...
            n = Math.min(n, this.buffer.size);
            n = this.buffer.read(buffer, n);
        }
        return n;
    }
    ...
    public String readUtf8Line() throws IOException {
        ...
        if (index == -1L) {
            if (this.buffer.size != 0L) {
                s = this.readUtf8(this.buffer.size);
            }
            else {
                s = null;
            }
            ...
            s = this.buffer.readUtf8Line(index);
        }
        return s;
    }
