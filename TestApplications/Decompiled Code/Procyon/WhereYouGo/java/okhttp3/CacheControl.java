    ...
    public static CacheControl parse(final Headers headers) {
        ...
        for (int i = 0; i < headers.size(); ++i, b = b8, b2 = b9, seconds = n2, seconds2 = n3, b3 = b10, b4 = b11, b5 = b12, seconds3 = n4, seconds4 = n5, b6 = b13, b7 = b14, s = s2, n = n6) {
            ...
            while (true) {
                ...
                int endIndex = HttpHeaders.skipUntil(value, beginIndex, "=,;");
                final String trim = value.substring(beginIndex, endIndex).trim();
                String s3;
                if (endIndex == value.length() || value.charAt(endIndex) == ',' || value.charAt(endIndex) == ';') {
                    ...
                    if (skipWhitespace < value.length() && value.charAt(skipWhitespace) == '\"') {
                        ...
                        s3 = value.substring(skipWhitespace, endIndex);
                        ++endIndex;
                    }
                    else {
                        endIndex = HttpHeaders.skipUntil(value, skipWhitespace, ",;");
                        s3 = value.substring(skipWhitespace, endIndex).trim();
                    }
                }
