...
public final class RSSExpandedReader extends AbstractRSSReader
{
    ...
    private FinderPattern parseFoundFinderPattern(final BitArray bitArray, final int n, final boolean b) {
        Label_0116: {
            ...
            ++n2;
            int n3 = this.startEnd[0] - n2;
            int nextUnset = this.startEnd[1];
            while (true) {
                final int[] decodeFinderCounters = this.getDecodeFinderCounters();
                System.arraycopy(decodeFinderCounters, 0, decodeFinderCounters, 1, decodeFinderCounters.length - 1);
                decodeFinderCounters[0] = n3;
                try {
                    return new FinderPattern(AbstractRSSReader.parseFinderValue(decodeFinderCounters, RSSExpandedReader.FINDER_PATTERNS), new int[] { n2, nextUnset }, n2, nextUnset, n);
                    n2 = this.startEnd[0];
                    nextUnset = bitArray.getNextUnset(this.startEnd[1] + 1);
                    n3 = nextUnset - this.startEnd[1];
                }
                catch (NotFoundException ex) {
                    return null;
                }
