...
public class LocusMapDataProvider implements MapDataProvider
{
    ...
    public static Waypoint locusMapWaypoint(final EventTable eventTable) {
        Waypoint waypoint;
        if (eventTable == null || !eventTable.isLocated() || !eventTable.isVisible()) {
            ...
            waypoint = new Waypoint(eventTable.name, location);
        }
        return waypoint;
    }
    ...
    public void addOther(final EventTable eventTable, final boolean b) {
        if (eventTable != null && eventTable.isLocated() && eventTable.isVisible()) {
            final Location location = new Location("");
            if (eventTable instanceof Zone && Preferences.GUIDING_ZONE_NAVIGATION_POINT == 1) {
                final Zone zone = (Zone)eventTable;
                location.setLatitude(zone.nearestPoint.latitude);
                location.setLongitude(zone.nearestPoint.longitude);
            }
            ...
            this.pack.addWaypoint(new Waypoint(eventTable.name, location));
        }
    }
