...
public class VectorMapDataProvider implements MapDataProvider
{
    ...
    @Override
    public void addCartridges(Vector<CartridgeFile> iterator) {
        if (iterator != null) {
            final MapPointPack e = new MapPointPack(false, 2130837561);
            iterator = ((Vector<CartridgeFile>)iterator).iterator();
            while (iterator.hasNext()) {
                final CartridgeFile cartridgeFile = iterator.next();
                if (cartridgeFile.latitude % 360.0 != 0.0 || cartridgeFile.longitude % 360.0 != 0.0) {
                    final MapPoint mapPoint = new MapPoint(cartridgeFile.name, cartridgeFile.description, cartridgeFile.latitude, cartridgeFile.longitude);
    ...
    public void addZone(final Zone zone, final boolean b) {
        if (zone != null && zone.isLocated() && zone.isVisible()) {
            ...
            if (Preferences.GUIDING_ZONE_NAVIGATION_POINT == 1 || zone.position == null) {
                e2.getPoints().add(new MapPoint(zone.name, zone.description, zone.nearestPoint.latitude, zone.nearestPoint.longitude, b));
            }
            else {
                e2.getPoints().add(new MapPoint(zone.name, zone.description, zone.position.latitude, zone.position.longitude, b));
            }
            if (b) {
