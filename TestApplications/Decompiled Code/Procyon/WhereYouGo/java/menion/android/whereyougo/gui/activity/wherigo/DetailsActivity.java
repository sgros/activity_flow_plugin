...
public class DetailsActivity extends MediaActivity implements IRefreshable, ILocationEventListener
{
    ...
    private void enableGuideOnEventTable() {
        final Location location = extractLocation(DetailsActivity.et);
        if (location != null) {
    ...
    private static Location extractLocation(final EventTable eventTable) {
        Location location;
        if (eventTable == null || !eventTable.isLocated()) {
            location = null;
        }
        else {
            final Location location2 = new Location("Details");
