...
class StrictLineReader implements Closeable {
    ...
    private int end;
    /* renamed from: in */
    private final InputStream f36in;
    private int pos;
    ...
    public String readLine() throws IOException {
        synchronized (this.f36in) {
            if (this.buf != null) {
                ...
                C03881 c03881 = new ByteArrayOutputStream((this.end - this.pos) + 80) {
                ...
                loop1:
                while (true) {
                    c03881.write(this.buf, this.pos, this.end - this.pos);
                    this.end = -1;
                    fillBuf();
                    i = this.pos;
                    while (i != this.end) {
                ...
                if (i != this.pos) {
                    c03881.write(this.buf, this.pos, i - this.pos);
                }
                this.pos = i + 1;
                String byteArrayOutputStream = c03881.toString();
    ...
    private void fillBuf() throws IOException {
        int read = this.f36in.read(this.buf, 0, this.buf.length);
        if (read != -1) {
            this.pos = 0;
            this.end = read;
            return;
