...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    static class SyncQueueItem {
        ...
        static SyncQueueItem obtainMessage(int i, int i2, int i3) {
            return SyncQueueItem.obtainMessage(i, i2, i3, 0, 0, 0, null);
        }

        static SyncQueueItem obtainMessage(int i, int i2, Object obj) {
            return SyncQueueItem.obtainMessage(i, i2, 0, 0, 0, 0, obj);
        }
    }
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> mainThreadCallback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new C03141();
            ...
            class C03141 implements Runnable {
                ...
                public void run() {
                    SyncQueueItem next = C05301.this.mQueue.next();
                    while (next != null) {
                        switch (next.what) {
                            case 1:
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> backgroundCallback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new C03151();
            ...
            class C03151 implements Runnable {
                ...
                public void run() {
                    while (true) {
                        SyncQueueItem next = C05312.this.mQueue.next();
                        if (next != null) {
                            switch (next.what) {
                                case 1:
                                    C05312.this.mQueue.removeMessages(1);
                                    backgroundCallback.refresh(next.arg1);
                                    break;
                                case 2:
                                    C05312.this.mQueue.removeMessages(2);
                                    C05312.this.mQueue.removeMessages(3);
                        ...
                        C05312.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
