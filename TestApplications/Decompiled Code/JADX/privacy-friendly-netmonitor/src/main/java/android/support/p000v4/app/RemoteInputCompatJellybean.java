...
class RemoteInputCompatJellybean {
    ...
    static Bundle getResultsFromIntent(Intent intent) {
        intent = RemoteInputCompatJellybean.getClipDataIntentFromIntent(intent);
        if (intent == null) {
            return null;
        }
    ...
    static void addResultsToIntent(RemoteInput[] remoteInputArr, Intent intent, Bundle bundle) {
        Intent clipDataIntentFromIntent = RemoteInputCompatJellybean.getClipDataIntentFromIntent(intent);
    ...
    public static void addDataResultToIntent(RemoteInput remoteInput, Intent intent, Map<String, Uri> map) {
        ...
        for (Entry entry : map.entrySet()) {
            ...
            if (str != null) {
                Bundle bundleExtra = clipDataIntentFromIntent.getBundleExtra(RemoteInputCompatJellybean.getExtraResultsKeyForData(str));
                if (bundleExtra == null) {
                    bundleExtra = new Bundle();
                }
                bundleExtra.putString(remoteInput.getResultKey(), uri.toString());
                clipDataIntentFromIntent.putExtra(RemoteInputCompatJellybean.getExtraResultsKeyForData(str), bundleExtra);
            }
        }
        intent.setClipData(ClipData.newIntent(RemoteInput.RESULTS_CLIP_LABEL, clipDataIntentFromIntent));
    }
