...
public class FontResourcesParserCompat {
    ...
    L_0x000d:
        ...
        throw r3;
    L_0x0015:
        r3 = android.support.p000v4.content.res.FontResourcesParserCompat.readFamilies(r3, r4);
        return r3;
        */
    ...
    private static FamilyResourceEntry readFamily(XmlPullParser xmlPullParser, Resources resources) throws XmlPullParserException, IOException {
        TypedArray obtainAttributes = resources.obtainAttributes(Xml.asAttributeSet(xmlPullParser), C0006R.styleable.FontFamily);
        String string = obtainAttributes.getString(C0006R.styleable.FontFamily_fontProviderAuthority);
        String string2 = obtainAttributes.getString(C0006R.styleable.FontFamily_fontProviderPackage);
        String string3 = obtainAttributes.getString(C0006R.styleable.FontFamily_fontProviderQuery);
        int resourceId = obtainAttributes.getResourceId(C0006R.styleable.FontFamily_fontProviderCerts, 0);
        int integer = obtainAttributes.getInteger(C0006R.styleable.FontFamily_fontProviderFetchStrategy, 1);
        int integer2 = obtainAttributes.getInteger(C0006R.styleable.FontFamily_fontProviderFetchTimeout, DEFAULT_TIMEOUT_MILLIS);
            ...
            if (obtainTypedArray.length() > 0) {
                ...
                if ((obtainTypedArray.getResourceId(0, 0) != 0 ? 1 : 0) != 0) {
                    for (i = 0; i < obtainTypedArray.length(); i++) {
                        list.add(FontResourcesParserCompat.toByteArrayList(resources.getStringArray(obtainTypedArray.getResourceId(i, 0))));
                    }
                } else {
                    list.add(FontResourcesParserCompat.toByteArrayList(resources.getStringArray(i)));
                }
            }
    ...
    private static FontFileResourceEntry readFont(XmlPullParser xmlPullParser, Resources resources) throws XmlPullParserException, IOException {
        TypedArray obtainAttributes = resources.obtainAttributes(Xml.asAttributeSet(xmlPullParser), C0006R.styleable.FontFamilyFont);
        int i = obtainAttributes.getInt(C0006R.styleable.FontFamilyFont_fontWeight, NORMAL_WEIGHT);
        boolean z = true;
        if (1 != obtainAttributes.getInt(C0006R.styleable.FontFamilyFont_fontStyle, 0)) {
            z = false;
        }
        int resourceId = obtainAttributes.getResourceId(C0006R.styleable.FontFamilyFont_font, 0);
        String string = obtainAttributes.getString(C0006R.styleable.FontFamilyFont_font);
        obtainAttributes.recycle();
        while (xmlPullParser.next() != 3) {
            FontResourcesParserCompat.skip(xmlPullParser);
        }
