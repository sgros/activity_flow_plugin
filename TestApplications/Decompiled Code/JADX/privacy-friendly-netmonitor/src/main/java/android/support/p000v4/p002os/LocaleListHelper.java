...
final class LocaleListHelper {
    ...
    private static int matchScore(Locale locale, Locale locale2) {
        ...
        if (!locale.getLanguage().equals(locale2.getLanguage()) || LocaleListHelper.isPseudoLocale(locale) || LocaleListHelper.isPseudoLocale(locale2)) {
            return 0;
        }
        String likelyScript = LocaleListHelper.getLikelyScript(locale);
        if (!likelyScript.isEmpty()) {
            return likelyScript.equals(LocaleListHelper.getLikelyScript(locale2));
        }
    ...
    private int findFirstMatchIndex(Locale locale) {
        for (int i = 0; i < this.mList.length; i++) {
            if (LocaleListHelper.matchScore(locale, this.mList[i]) > 0) {
                return i;
            }
        }
    ...
    L_0x002b:
        ...
        r2 = android.support.p000v4.p002os.LocaleHelper.forLanguageTag(r2);
    ...
    static boolean isPseudoLocalesOnly(@Nullable String[] strArr) {
        ...
        for (String str : strArr) {
            if (!str.isEmpty() && !LocaleListHelper.isPseudoLocale(str)) {
                return false;
            }
        }
        return true;
