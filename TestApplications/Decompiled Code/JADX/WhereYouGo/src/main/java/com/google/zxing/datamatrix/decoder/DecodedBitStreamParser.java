...
final class DecodedBitStreamParser {
    ...
    L_0x0043:
        ...
        return r6;
    L_0x0047:
        r6 = com.google.zxing.datamatrix.decoder.DecodedBitStreamParser.C01871.f18xb73eb560;
        r7 = r2.ordinal();
        r6 = r6[r7];
        switch(r6) {
    ...
    private static void decodeBase256Segment(BitSource bits, StringBuilder result, Collection<byte[]> byteSegments) throws FormatException {
        ...
        } else if (d1 < MapsforgeActivity.FILE_SYSTEM_CACHE_SIZE_DEFAULT) {
            count = d1;
            codewordPosition = codewordPosition2;
        } else {
            codewordPosition = codewordPosition2 + 1;
            count = ((d1 - 249) * MapsforgeActivity.FILE_SYSTEM_CACHE_SIZE_DEFAULT) + unrandomize255State(bits.readBits(8), codewordPosition2);
        }
        if (count < 0) {
            throw FormatException.getFormatInstance();
        }
