...
import android.support.p000v4.app.BackStackRecord.C0011Op;
...
class FragmentTransition {
    ...
    public static void calculateFragments(BackStackRecord transaction, SparseArray<FragmentContainerTransition> transitioningFragments, boolean isOptimized) {
        ...
        for (int opNum = 0; opNum < numOps; opNum++) {
            FragmentTransition.addToFirstInLastOut(transaction, (C0011Op) transaction.mOps.get(opNum), transitioningFragments, false, isOptimized);
        }
    }
    ...
    public static void calculatePopFragments(BackStackRecord transaction, SparseArray<FragmentContainerTransition> transitioningFragments, boolean isOptimized) {
        if (transaction.mManager.mContainer.onHasView()) {
            for (int opNum = transaction.mOps.size() - 1; opNum >= 0; opNum--) {
                FragmentTransition.addToFirstInLastOut(transaction, (C0011Op) transaction.mOps.get(opNum), transitioningFragments, true, isOptimized);
            }
        }
    }

    private static void addToFirstInLastOut(BackStackRecord transaction, C0011Op op, SparseArray<FragmentContainerTransition> transitioningFragments, boolean isPop, boolean isOptimizedTransaction) {
