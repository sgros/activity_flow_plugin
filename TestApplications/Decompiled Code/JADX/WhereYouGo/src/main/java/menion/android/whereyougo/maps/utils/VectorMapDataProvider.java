...
public class VectorMapDataProvider implements MapDataProvider {
    ...
    public void addZone(Zone z, boolean mark) {
        if (z != null && z.isLocated() && z.isVisible()) {
            ...
            MapPointPack pack = new MapPointPack();
            if (Preferences.GUIDING_ZONE_NAVIGATION_POINT == 1 || z.position == null) {
                pack.getPoints().add(new MapPoint(z.name, z.description, z.nearestPoint.latitude, z.nearestPoint.longitude, mark));
            } else {
                pack.getPoints().add(new MapPoint(z.name, z.description, z.position.latitude, z.position.longitude, mark));
            }
            if (mark) {
                pack.setResource(C0254R.C0252drawable.marker_green);
            } else {
