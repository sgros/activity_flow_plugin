...
public class GifAnimationMetaData implements Serializable, Parcelable {
    public static final Creator<GifAnimationMetaData> CREATOR = new C03771();
    ...
    /* renamed from: pl.droidsonroids.gif.GifAnimationMetaData$1 */
    static class C03771 implements Creator<GifAnimationMetaData> {
        C03771() {
        }

        public GifAnimationMetaData createFromParcel(Parcel source) {
            return new GifAnimationMetaData(source, null);
        }
    ...
    public GifAnimationMetaData(@NonNull Resources res, @RawRes @DrawableRes int id) throws NotFoundException, IOException {
    ...
    @Beta
    public long getDrawableAllocationByteCount(@Nullable GifDrawable oldDrawable, @IntRange(from = 1, to = 65535) int sampleSize) {
        if (sampleSize < 1 || sampleSize > SupportMenu.USER_MASK) {
    ...
    public String toString() {
        ...
        String suffix = String.format(Locale.ENGLISH, "GIF: size: %dx%d, frames: %d, loops: %s, duration: %d", new Object[]{Integer.valueOf(this.mWidth), Integer.valueOf(this.mHeight), Integer.valueOf(this.mImageCount), loopCount, Integer.valueOf(this.mDuration)});
